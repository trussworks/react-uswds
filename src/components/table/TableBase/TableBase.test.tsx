...
describe('Table component', () => {
  it('renders without errors', () => {
    const { getByRole } = render(<TableBase>{testContent}</TableBase>)

    expect(getByRole('table')).toBeInTheDocument()
  })

  it('renders striped table', () => {
    const { getByRole } = render(<TableBase striped>{testContent}</TableBase>)

    expect(getByRole('table')).toHaveClass('usa-table usa-table--striped')
  })

  it('renders compact table', () => {
    const { getByRole } = render(<TableBase compact>{testContent}</TableBase>)

    expect(getByRole('table')).toHaveClass('usa-table usa-table--compact')
  })

  it('renders full width table', () => {
    const { getByRole } = render(<TableBase fullWidth>{testContent}</TableBase>)
  ...
  it('renders borderless stacked table', () => {
    const { getByRole } = render(
      <TableBase stackedStyle="default">{testContent}</TableBase>
    )
  ...
  it('renders stacked table with border and headers', () => {
    const { getByRole } = render(
      <TableBase stackedStyle="headers" bordered>
        {testContent}
      </TableBase>
    )
  ...
  it('passes the class onto the root table element', () => {
    const { getByRole } = render(
      <TableBase className="custom-class">{testContent}</TableBase>
    )
