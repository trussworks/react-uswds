import { CustomizableFilter } from '../combobox/ComboBox/ComboBox.js';

export declare const DEFAULT_MAX_TIME = "23:59";
export declare const DEFAULT_MAX_TIME_MINUTES: number;
export declare const DEFAULT_MIN_TIME = "00:00";
export declare const DEFAULT_MIN_TIME_MINUTES = 0;
export declare const DEFAULT_STEP = 30;
export declare const MIN_STEP = 1;
export declare const TIME_PICKER_CUSTOM_FILTER: CustomizableFilter;
