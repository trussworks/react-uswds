...
6. If the npm install in that process was not successful (should look like the image above), run `npm install`. Wait until it is done before you do anything. Takes ~5ish minutes
7. If it is successful run `npm run storybook`. this might also take a couple of minutes to complete.
8. When the link that Storybook will be hosted on is available, you will get a message saying your application is running on port 9009 in the bottom right corner. click the `Open in Browser` button
